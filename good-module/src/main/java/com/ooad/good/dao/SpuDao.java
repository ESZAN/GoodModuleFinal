...
//            int ret = roleMapper.updateByPrimaryKeySelective(rolePo);
    ...
    public SpuPo getSpuBySpuId(Long id) {
        SpuPo po=new SpuPo();
        ReturnObject<Object> retObj = null;
        SpuPoExample spuPo = new  SpuPoExample();
        SpuPoExample.Criteria criteria = spuPo.createCriteria();
        criteria.andIdEqualTo(id);
        try {
            po = spuPoMapper.selectByPrimaryKey(id);
        } catch (Exception e) {
            logger.error("getSpu id= ",id," : DataAccessException:" + e.getMessage());
            return null;
        }
        if(po==null)
        {
            logger.info("spu不存在或已被删除：id = " + id);
            return null;
        }
        return po;
    }
    ...
    {
        SpuPo po=spuPoMapper.selectByPrimaryKey(id);
