...
/*
    ...
    public ReturnObject<Object> modifyShopByVo(Long id, ShopVo shopVo) {
        // 查询密码等资料以计算新签名
        if(shopVo.getName()==null||shopVo.getName().isBlank())
        {
            logger.info("无法使用空店铺名");
...
*/
    ...
    public ReturnObject auditShopInfo(Long id,Boolean conJudge) {
        ...
        try {
            ret=shopMapper.updateByPrimaryKeySelective(newPo);
            if(conJudge)
            logger.info("店铺通过审核,店铺 id = " + id + " 的状态修改为 " + Shop.StateType.getTypeByCode(newPo.getState().intValue()).getDescription());
            else
                logger.info("店铺未通过审核，店铺 id = " + id + " 的状态修改为 " + Shop.StateType.getTypeByCode(newPo.getState().intValue()).getDescription());
            retObj = new ReturnObject<>(ResponseCode.OK);
        }
        catch (DataAccessException e) {
            // 数据库错误
    ...
    public ReturnObject onshelvesShop(Long id) {
        ...
        try {
            ret=shopMapper.updateByPrimaryKeySelective(newPo);
                logger.info("店铺 id = " + id + " 的状态修改为 " + Shop.StateType.getTypeByCode(newPo.getState().intValue()).getDescription());
                retObj = new ReturnObject<>(ResponseCode.OK);
        }
        catch (DataAccessException e) {
            // 数据库错误
     ...
     * @return
     */
    public ShopPo getSimpleShopByShopId(Long id) {
        ShopPo po=new ShopPo();
        ReturnObject<Object> retObj = null;
        ShopPoExample shopPo = new  ShopPoExample();
        ShopPoExample.Criteria criteria = shopPo.createCriteria();
