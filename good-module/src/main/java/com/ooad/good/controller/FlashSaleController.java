...
public class FlashSaleController {
    ...
    public Object insertFlashSale(@Validated @RequestBody FlashSaleVo vo, BindingResult bindingResult,
        ...
        flashSale.setTimeSeq(id);
        Byte b=0;
        flashSale.setState(b);//处于下线状态

        ReturnObject retObject = flashsaleService.insertFlashsale(flashSale);
        if (retObject==null||retObject.getData() != null) {
    ...
    public Flux<FlashSaleItemRetVo> getCurentFlashSales(){

        Long id = 9L;
        return flashsaleService.queryTopicsByTime(id);
    ...
    @ApiOperation(value = "向秒杀活动添加商品", produces = "application/json;charset=UTF-8")
    @ApiImplicitParams({
            @ApiImplicitParam(paramType = "header", dataType = "String", name = "authorization", value = "Token", required = true),
    ...
    public Object deleteflashSale(@PathVariable("did")Long did,@PathVariable("id")Long id){
        logger.info("delete flashsale: id =" +id);
        // 校验前端数据
     /*   Object retObject = Common.processFieldErrors(bindingResult, httpServletResponse);
        if (retObject != null) {
